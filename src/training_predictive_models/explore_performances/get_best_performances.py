...
print("Get average by algorithm, group, and FFT application")
matrix_data = []
for i in range(8):
    name_group = "group_{}".format(i)
    print("Processing group: ", name_group)

    for apply_fft in ["FFT", "NO-FFT"]:
        filter_df = df_performances.loc[(df_performances['id_group'] == name_group) & (df_performances['apply_FFT'] == apply_fft)]
        ...
        for j in range(len(unique_algorithm)):
            ...
            response_average.insert(0, apply_fft)
...
for i in range(8):
    name_group = "group_{}".format(i)
    filter_df_name = df_data_average.loc[(df_data_average['group_id'] == name_group)]
    filter_df_name.reset_index(inplace=True)
    for apply_fft in ["FFT", "NO-FFT"]:
        filter_df = filter_df_name.loc[(df_data_average['apply_FFT'] == apply_fft)]
        filter_df.reset_index(inplace=True)
        
        dict_data = {}
        for performance in columns_performances:

            if performance not in ["algorithm", "group_id", 'apply_FFT']:
                dict_performance = {}
                list_description = []
        ...
        name_key = "{}-{}".format(name_group, apply_fft)
